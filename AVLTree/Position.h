...
//Destructor, destrueix els fills primer per destruir l'arbre fent un recorregut de post-ordre. Cost per Position O(1), cost total O(n) (per eliminar l'arrel, i l'arbre sencer, s'eliminen abans els n descendents)
...
//Es calcula recursivament la profunditat de la Position, es retorna 0 si és l'arrel de l'arbre. Cost O(log n) (alçada de l'arbre)
...
//Es retorna l'alçada de la Position. Cost O(1)
template <class K, class V> int Position<K,V>::height() const {
    return _height;
}

//Es retorna el factor de balanç de la Position. Cost O(1)
template <class K, class V> int Position<K,V>::balance() const {
    return _balance;
}
...
//Es calcula l'alçada de la Position. Cost O(1)
template <class K, class V> void Position<K,V>::calculateHeight() {
    int esq = (left() == nullptr)? 0 : left()->height();
    int dre = (right() == nullptr)? 0 : right()->height();
        
    _height = 1 + ((esq >= dre)? esq : dre);
}

//Es calcula el factor de balanç de la Position. Cost O(1)
template <class K, class V> void Position<K,V>::calculateBalance() {
    int dre = (right() == nullptr)? 0 : right()->height();
    int esq = (left() == nullptr)? 0 : left()->height();
